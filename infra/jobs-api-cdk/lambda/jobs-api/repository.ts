...
export const listJobs = async (query: { limit?: string; offset?: string; status?: string }) => {
...
export const getJobDetails = async (jobId: string) => {
  const out = await ddb.send(new GetCommand({ TableName: config.tableName, Key: { uuid: jobId } }));
