...
const apiBaseUrl =
  process.env.NEXT_PUBLIC_JOBS_API_BASE_URL?.replace(/\/$/, "") ?? "http://localhost:8787";

export const jobsApi = createApi({
  reducerPath: "jobsApi",
  baseQuery: fetchBaseQuery({ baseUrl: apiBaseUrl }),
  endpoints: (builder) => ({
    listJobs: builder.query<ListJobsResponse, { limit?: number; offset?: number; status?: string } | void>({
      query: (params) => ({
        url: "/v1/jobs",
